...
class Grid_long_stoptrail():
    ...
    def __init__(self):
        ...
        # 策略所需参数
        self.stoptrail_triger_percent = 1.012
        self.stoptrail_space = 0.008
    ...
    def run(self):
        ...
        while True:
            ...
            if self.current_hour != self.last_hour:
                message = 'Time: %s , main thread running normal, current total target coin position is %s' %(time.strftime("%Y-%m-%d %H:%M:%S"),self.current_total_target_coin)
                self.dingding_notice(message)
                print(message)
            ...
            if self.format_amount(self.current_total_target_coin) <= 0.000002:
                ...
                message = '策略 %s 新的循环开始 \n初始的目标标的可用量: %s \n初始的目标标的冻结量: %s \n初始的目标标的总量: %s' \
                          '\n初始的USDT可用量: %s \n初始的USDT冻结量: %s \n初始的USDT总量: %s \n当地时间：%s' \
                          % (self.strategy_name, self.format_amount(self.initial_target_coin_trade),
                             self.format_amount(self.initial_target_coin_frozen),
                             self.format_amount(self.initial_total_target_coin),
                             self.format_amount(self.initial_base_coin_trade),
                             self.format_amount(self.initial_base_coin_frozen),
                             self.format_amount(self.initial_total_base_coin),
            ...
            # 如果仓位大于0，则判断是否出发追踪止损
                ...
                if position_ratio < 0.05:
                    self.stoptrail_triger_percent = 1.0402
                elif position_ratio < 0.1:
                    self.stoptrail_triger_percent = 1.0302
                elif position_ratio < 0.3:
                    self.stoptrail_triger_percent = 1.0274
                elif position_ratio < 0.4:
                    self.stoptrail_triger_percent = 1.0214
                else:
                ...
                if self.current_hour != self.last_hour:
                    message = 'Time: %s, average cost is: %s, current price is: %s, stoptrail triger point is: %s, stoptrail trigger percent is: %s, stoptrail_trail_ing is: %s. ' \
                              % (time.strftime("%Y-%m-%d %H:%M:%S"), avg_cost, self.new_price,
                                 avg_cost * self.stoptrail_triger_percent,self.stoptrail_triger_percent, self.stop_trial_ing)
                    self.dingding_notice(message)
                    print(message)
